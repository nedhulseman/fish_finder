...
class Bounder(tk.Tk):
    ...
    def startBbox(self):
        ...
        '''
        self.bbox_coords = {}
        columns = ['xmin', 'ymin', 'xmax', 'ymax', 'label', 'image',
                   'path']
        for val in columns:
            self.bbox_coords[val] = None
        self.bbox_coords['path'] = os.path.join(self.images_path, self.current_image_name)
        self.bbox_coords['image'] = self.current_image_name
    ...
    def nextImage(self):
        ...
        if self.images_path == None:
            ...
            '''
            self.bbox_coords['xmin'] = self.scaledBbox[0]
